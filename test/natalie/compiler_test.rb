...
describe 'Natalie::Compiler' do
  ...
  describe 'Pass4' do
    it 'compiles from pass3 AST to pass4 AST' do
      ...
      Value *obj_exception(Env *env, Value *self);
Value *obj_array(Env *env, Value *self);
Value *obj_comparable(Env *env, Value *self);
Value *obj_dir(Env *env, Value *self);
Value *obj_enumerable(Env *env, Value *self);
Value *obj_enumerator(Env *env, Value *self);
Value *obj_errno(Env *env, Value *self);
Value *obj_io(Env *env, Value *self);
Value *obj_struct(Env *env, Value *self);
obj_exception(env, self);
obj_array(env, self);
obj_comparable(env, self);
obj_dir(env, self);
obj_enumerable(env, self);
obj_enumerator(env, self);
obj_errno(env, self);
obj_io(env, self);
obj_struct(env, self);
